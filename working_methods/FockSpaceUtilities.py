...
class FockSpaceUtilities():
    ...
    def __init__(self, norb: int, nelec: Optional[int | Sequence[int]]):
        """Initialize Fock space utilities with fixed number of orbitals and electrons.
            ...
            Args:
                norb: Number of orbitals (must be positive)
                nelec: Number of electrons (int), allowed range (sequence), or None for all
        """
    ...
    def create_normalized_vector(self, values_to_activate: dict[int, int]) -> np.ndarray:
            """
            Builds a normalized vector with the elements indicated in values_to_activate
            """
            vector = np.zeros(len(self.basis))
            
            for i, element in enumerate(self.basis):
    ...
    @staticmethod
    def classify_eigenstate(
        preferred_basis: List[np.ndarray],
        correspondence_with_determinant_basis: Dict[int, str],
        eigenstate: np.ndarray
    ) -> Dict:
        """Classify eigenstate in terms of preferred basis."""
        probabilities = []
        for vector in preferred_basis:
            overlap = np.vdot(vector, eigenstate)
            probability = np.abs(overlap) ** 2
            probabilities.append(probability)

        max_index = np.argmax(probabilities)
        max_label = correspondence_with_determinant_basis[max_index]
        max_probability = probabilities[max_index]

        sorted_labels_and_probs = sorted(
            zip(correspondence_with_determinant_basis.values(), probabilities),
            key=lambda x: x[1],
            reverse=True
        )
