...
def simulateCurrentAtTWait(fixedParameters, initialStateLabel, tWait, totalPoints):
...
def runSweepOverTWait(fixedParameters, initialStateLabel, tWaitList, totalPoints=500):

    nCores = multiprocessing.cpu_count()
    print(f"Ejecutando en paralelo con joblib usando {nCores} núcleos...")

    resultsCurrent = Parallel(n_jobs=nCores)(
        delayed(simulateCurrentAtTWait)(fixedParameters, initialStateLabel, tWait, totalPoints)
        for tWait in tWaitList
    )

    # === Gráfica corriente vs tiempo de espera ===
    plt.figure(figsize=(8, 5))
    plt.plot(tWaitList, resultsCurrent, 'o-', color='crimson')
    plt.xlabel("tWait (ns)")
    plt.ylabel("Current (normalized)")
    plt.title("Current vs Wait Time")
    plt.grid(True)
    ...
    figPath = os.path.join(outputDir, f"Current_vs_tWait_JOBLIB_{timestamp}.png")
    plt.savefig(figPath)
    print(f"Figura guardada en: {figPath}")

    dataPath = os.path.join(outputDir, f"Current_vs_tWait_JOBLIB_{timestamp}.txt")
    np.savetxt(dataPath, np.column_stack((tWaitList, resultsCurrent)), header="tWait(ns)\tCurrent(normalized)")
...
# === EJECUTAR ===
tWaitValues = np.linspace(0.1, 2.0, 12)  # en ns
runSweepOverTWait(fixedParameters, initialStateLabel="LR,T+,T-", tWaitList=tWaitValues)
