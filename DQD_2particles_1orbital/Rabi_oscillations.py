...
def runDynamics(fixedParameters, initialStateLabel, useCollapseOperators=True):
    ...
    H_isolated = dqd.project_hamiltonian(dqd.singlet_triplet_basis, parameters_to_change=fixedParameters)

    inverseCorrespondence = {v: k for k, v in dqd.singlet_triplet_correspondence.items()}

    H_qutip = Qobj(H_isolated)
    ...
    if useCollapseOperators:
        ...
        for i in range(8):
            ...
            LProjected = dqd.project_hamiltonian(dqd.singlet_triplet_basis, alternative_operator=LFockBasis)
    ...
    statesToPlot = ["LR,T+,T-", "LL,S,T-", "LR,S,T-"]
