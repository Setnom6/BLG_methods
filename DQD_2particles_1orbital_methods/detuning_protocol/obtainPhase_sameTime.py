...
if __name__ == "__main__":
    ...
    topValues = np.linspace(interactionDetuning*1.001, DM.fixedParameters[DQDParameters.U0.value], 120)

    minimalPlateauTime = 1.0*expectedPeriod
    maxSlopeTime = computeSlopeTime(interactionDetuning, topValues[-1])
    totalPhaseAccumulationTime = 2*maxSlopeTime+minimalPlateauTime

    slopeTimes = []
    plateauTimes = []

    for tv in topValues:
        # Total interaction time should be same for all topValues, therefore Plateau time should change to adapt to fixed slopes
        slopeTime = computeSlopeTime(interactionDetuning,tv)
        slopeTimes.append(slopeTime)
        plateauTimes.append(totalPhaseAccumulationTime-2.0*slopeTime)

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 8))

    maxCores = min(24, cpu_count())
    logging.info(f"Running using {maxCores} cores.")
    results = Parallel(n_jobs=maxCores)(delayed(runSingleFactor)(DM, expectedPeriod, interactionDetuning, slopeTimes[idx], plateauTimes[idx], topValues[idx]) 
                                      for idx in range(len(topValues)))

    resultsP0, resultsP1, resultsPhi = zip(*results)
    xValues = topValues/interactionDetuning  # normalizado por interactionDetunig
    ...
    ax1.set_xlabel("Peak reached / interaction detuning")
    ax1.set_ylabel("Final Population")
    ax1.legend()
    ax1.grid()

    ax2.set_xlabel("Peak reached / interaction detuning")
    ax2.set_ylabel("Phi (grad)")
    ax2.set_ylim(0, 180)
    ax2.legend()
    ax2.grid()

    fig.suptitle(f"Slope: 0.5 meV/ns, total phase accumulation time: {totalPhaseAccumulationTime:.3f} ns")

    plt.tight_layout()
    DM.saveResults(name="Phase_accumulation_topValues")
