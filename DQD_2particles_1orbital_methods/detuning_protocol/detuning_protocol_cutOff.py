...
totalPoints = 300
N = 5 # states to keep in naif cutoff

populations, tlistNano, eiValues = DM.detuningProtocol(intervalTimes, totalPoints, cutOffN=N)

fig, (ax1, ax2, ax3) = plt.subplots(nrows=3, sharex=True, figsize=(10, 10), height_ratios=[3, 1, 1])

# Individual populations
statesToPlot = [DM.correspondence[i] for i in range(N)]
for label in statesToPlot:
    index = DM.invCorrespondence[label]
    ax1.plot(tlistNano, populations[:, index], label=label)
ax1.set_ylabel('Population')
ax1.set_title(f'Population dynamics (cutOff for {N} states with detuning sweep)')
