...
def runDynamics(bx, parameters, times, cutOffN):
...
if __name__ == "__main__":
    ...
    for idx, ei in enumerate(detuningList):
        ...
        currents = Parallel(n_jobs=numCores)(
            delayed(runDynamics)(bx, parameters, timesNs, cutOffN)
            for bx in bxList
        )
        ...
        plt.xlabel("Time (ns)")
        plt.ylabel("bx (T)")
        plt.title(f"Current vs bx and interaction time for {cutOffN} first states, " + 
        ...
        DM.saveResults(name="rabi_2D_bx_cutOff")
