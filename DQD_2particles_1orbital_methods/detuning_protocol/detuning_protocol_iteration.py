...
if __name__ == "__main__":
    ...
    # Definir el rango de interactionTime que quieres estudiar (en ns)
    nPoints = 3
    interactionTimesToScan = np.linspace(expectedPeriod*4.5, expectedPeriod*5.5, nPoints)  # Ajusta el rango según interés
    ...
    for idx, interactionTime in enumerate(interactionTimesToScan):
        # Reconstruir el protocolo con el nuevo interactionTime
        ...
        # Guardar la población final de los estados de interés
        ...
        sumSinglet_list.append(sumSinglet)
        sumTriplet_list.append(sumTriplet)
        finalPopulations.append(populations[-1, :])  # store full state vector 

        logging.info(f"Simulation {idx+1}/{len(interactionTimesToScan)} completed.\n")
    ...
    for label in statesToPlot:
        ...
        ax2.plot(interactionTimesMultiples, finalPopulations[:, statesToPlot.index(label)], marker='o', label=label)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(DM.figuresDir, 'sensitivity_interaction_time.png'))
